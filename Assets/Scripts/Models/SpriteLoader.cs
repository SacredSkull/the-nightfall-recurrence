...
namespace Models {
	public class SpriteLoader {
	    ...
	    public void Load(MapItem mi) {
			...
			if (sprite == null) {
				Logger.UnityLog($"[SPRITE] Could not find/load a sprite for {mi.string_id}", Logger.Level.ERROR);
				return;
			}
	        ...
	        if(tool != null) {
                ...
                if (sprite == null) {
                    Logger.UnityLog($"[SPRITE] Could not find/load a tail sprite for {mi.string_id}", Logger.Level.ERROR);
                    return;
                }
