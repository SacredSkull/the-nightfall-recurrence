...
namespace Models {
    ...
    public class LevelModel : DatabaseLoader {
        ...
        public override void Load() {
	        ...
	        Logger.UnityLog("Wasn't given a level to load!", LogLevels.WARNING);
	        Load(path);
        }
		...
		private void LoadMap() {
            ...
            Logger.UnityLog("Starting to parse Tilesets", LogLevels.INFO);
            ...
            Logger.UnityLog("Finished parsing Tilesets", LogLevels.INFO);
            ...
            foreach (Layer l in map.layers) {
                if (l.name == "Geometry") {
                    ...
                    Logger.UnityLog("Finished parsing Geometry layer", LogLevels.INFO);
                    ...
                    for (int row = 0; row < l.height; row++) {
                        ...
                        for (int column = 0; column < l.width; column++) {
	                        ...
	                        var clone = MapItem.Factory.Create(mi);
                    ...
                    Logger.UnityLog("Finished parsing Entity layer", LogLevels.INFO);
                }
            }
			...
			for (int row = 0; row < Math.Max(EntityGrid.Height, GeometryGrid.Height); row++) {
				...
				for (int column = 0; column < Math.Max(EntityGrid.Width, GeometryGrid.Width); column++) {
					MapItem clone = MapItem.Factory.CreateBlankTile();
	        ...
	        if (mapItems.Count == 0)
		        Logger.UnityLog("mapItems does not contain anything!", LogLevels.ERROR);
